...
export function AnalyticsDashboard() {
  ...
  const [subTab, setSubTab] = useState<"overview" | "spending" | "investments" | "cashflow" | "networth">("overview");
  ...
  return (
    <div>
      ...
      {/* Sub-tabs */}
      <div className="flex overflow-x-auto mb-6" style={{ borderBottom: "1px solid var(--border)", scrollbarWidth: "none" }}>
        {SUB_TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setSubTab(t.id)}
            className="py-[10px] px-4 text-[12px] font-semibold whitespace-nowrap flex-shrink-0"
            style={{
              color: subTab === t.id ? "var(--green)" : "var(--muted)",
              borderBottom: subTab === t.id ? "2px solid var(--green)" : "2px solid transparent",
              background: "transparent",
              border: "none",
              borderBottomStyle: "solid",
              borderBottomWidth: 2,
              borderBottomColor: subTab === t.id ? "var(--green)" : "transparent",
              cursor: "pointer",
            }}
          >
            {t.label}
      ...
      {subTab === "overview" && (
        <div>
          ...
          <div className="rounded-[16px] p-6 mb-6" style={{ background: "linear-gradient(135deg,var(--navy2),var(--navy))", color: "#fff" }}>
            <div className="flex items-start justify-between mb-5 flex-wrap gap-4">
              <div className="flex-1">
                ...
                <div className="text-[12px]" style={{ color: "rgba(255,255,255,.65)", lineHeight: 1.6 }}>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button className="px-4 py-[9px] rounded-[10px] text-[12px] font-medium border" style={{ color: "rgba(255,255,255,.75)", borderColor: "rgba(255,255,255,.2)", background: "transparent" }}>⚡ Pay Off Credit Card Now</button>
              <button className="px-4 py-[9px] rounded-[10px] text-[12px] font-medium border" style={{ color: "rgba(255,255,255,.75)", borderColor: "rgba(255,255,255,.2)", background: "transparent" }} onClick={() => setSubTab("spending")}>See Spending Detail →</button>
            </div>
          </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 500 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid var(--border)" }}>
                    {["Category", ...catVisibleMonths, "Trend", `${catVisibleCount}M Total`, ""].map((h) => (
                      <th key={h} style={{ padding: "8px 12px", textAlign: h === "Category" || h === "" ? "left" : "right", fontSize: 10, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".5px" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {CAT_TREND_ROWS.map((row) => {
            ...
            <ChartCard title="Recent Transactions">
              ...
              <div className="flex flex-col">
                {filteredTxns.map((txn) => (
                  <div key={txn.name + txn.date} className="flex items-center gap-3 py-3" style={{ borderBottom: "1px solid var(--border)" }}>
      ...
      {subTab === "investments" && (
        <div>
          {/* Portfolio KPIs */}
          <div className="grid gap-4 mb-5" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))" }}>
          ...
          <div className="grid gap-4 mb-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))" }}>
      ...
      {subTab === "cashflow" && (
        <div>
          <div className="grid gap-4 mb-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))" }}>
            <Kpi label="Total In (6M)" value="₦2.7M" delta="↑ Salary consistent" deltaDir="up" />
            <Kpi label="Total Out (6M)" value="₦1.74M" delta="↑ +₦8k vs prior period" deltaDir="up" />
            <Kpi label="Net Cash Flow" value="+₦960k" delta="↑ Best 6-month streak" deltaDir="up" />
            <Kpi label="Avg Monthly Surplus" value="₦160k" delta="↑ Available to invest" deltaDir="up" />
          </div>

          <ChartCard
            title="Monthly Cash Flow"
            sub="Money in vs. money out"
            action="Ask buddy →"
            onAction={() => goToChat("Looking at my income vs spending over the last 6 months, what pattern do you see and what should I change?")}
          >
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={cashflowData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }} barGap={4}>
                <CartesianGrid strokeDasharray="4 4" stroke="var(--border)" />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: "var(--muted)" }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: "var(--muted)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `₦${v}k`} />
                <Tooltip formatter={(v) => `₦${v}k`} contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 11 }} />
                <Legend wrapperStyle={{ fontSize: 11, color: "var(--muted)" }} />
                <Bar dataKey="in" fill="#00C48C" fillOpacity={0.85} radius={[4, 4, 0, 0]} name="Cash In" />
                <Bar dataKey="out" fill="#E24B4A" fillOpacity={0.75} radius={[4, 4, 0, 0]} name="Cash Out" />
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>

          <div className="mt-4">
            <ChartCard title="Cashflow Breakdown — March 2026" sub="Where your money moved">
              <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>
                {[
                  { label: "Salary (GTBank)", amt: "+₦450,000", color: "var(--green2)", icon: "💰" },
                  { label: "Food & Dining", amt: "-₦82,000", color: "#E24B4A", icon: "🍔" },
                  { label: "Subscriptions", amt: "-₦34,000", color: "#E24B4A", icon: "🔄" },
                  { label: "Transport", amt: "-₦28,000", color: "#E24B4A", icon: "🚗" },
                  { label: "Shopping", amt: "-₦19,000", color: "#E24B4A", icon: "🛍️" },
                  { label: "ARM MMF (Investment)", amt: "-₦80,000", color: "#4A90D9", icon: "📈" },
                  { label: "Net Surplus", amt: "+₦207,000", color: "var(--green)", icon: "✅" },
                ].map((row) => (
                  <div key={row.label} className="flex items-center gap-3 px-3 py-3 rounded-[10px]" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                    <div className="text-[18px]">{row.icon}</div>
                    <div className="flex-1 text-[12px]" style={{ color: "var(--text)" }}>{row.label}</div>
                    <div className="text-[13px] font-semibold" style={{ color: row.color }}>{row.amt}</div>
                  </div>
                ))}
              </div>
            </ChartCard>
      ...
      {subTab === "networth" && (
        <div>
          <div className="grid gap-4 mb-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))" }}>
            <Kpi label="Current Net Worth" value="₦1.07M" delta="↑ +₦285k in 3 months" deltaDir="up" />
            <Kpi label="Total Assets" value="₦1.165M" delta="Investments + Savings" deltaDir="neutral" />
            <Kpi label="Total Liabilities" value="₦95k" delta="↓ Credit card only" deltaDir="neutral" />
            <Kpi label="Growth Rate (6M)" value="+91%" delta="↑ Strong trajectory" deltaDir="up" />
          </div>
                  ...
                  <div className="flex flex-col gap-2">
                    {[
                      ...
                      <div key={a.name} className="flex items-center gap-2">
                        <div className="text-[12px] flex-shrink-0" style={{ color: "var(--text)", width: 130 }}>{a.name}</div>
                        <div className="flex-1 h-[6px] rounded-full overflow-hidden" style={{ background: "var(--bg)" }}>
                          <motion.div
                            className="h-full rounded-full"
                  ...
                  <div className="flex flex-col gap-2">
                    {[{ name: "GTBank Credit Card", value: "₦95,000", pct: 100 }].map((l, i) => (
                      <div key={l.name} className="flex items-center gap-2">
                        <div className="text-[12px] flex-shrink-0" style={{ color: "var(--text)", width: 130 }}>{l.name}</div>
                        <div className="flex-1 h-[6px] rounded-full overflow-hidden" style={{ background: "var(--bg)" }}>
                          <motion.div
                            className="h-full rounded-full"
